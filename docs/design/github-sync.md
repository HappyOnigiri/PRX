...
畳み込んだレビュー判定に加えて、未応答のレビュー依頼が残っているか、変更要求レビューの最新提出時刻、最新コミットの push 時刻を取得する。
...
GraphQL の push 時刻は null になりうるので、コミット日時にフォールバックする。REST には push 時刻がないため常にコミット日時を使う。
REST への fallback でも同じ 3 項目を埋める。どちらの経路で取得しても表示は一致させる。
