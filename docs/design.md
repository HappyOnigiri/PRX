...
Every response-producing CLI command uses schema version `2`. A successful command returns an envelope such as `{"schema_version":"2","ok":true,"data":{...}}`; `data` is always a JSON object. Existing object payloads keep their fields, while collection responses use named fields: `features`, `tasks`, `dependencies`, `pull_requests`, `documents`, `ready_tasks`, `review_waiting_tasks`, `conflict_tasks`, `stale_tasks`, `hosts`, or `auth_methods` as appropriate. A failed command returns `{"schema_version":"2","ok":false,"error":"..."}` with a non-empty top-level error string and no `data`; a successful response has no `error`. The process exits `0` on success and `1` on command failure. Domain error codes remain available inside the application and ConnectRPC contracts, but are not part of the CLI envelope.

Normal output and `--json` have the same response shape. Normal output is indented JSON and `--json` is compact JSON; both keep the response on stdout, while warnings and server logs go to stderr. Help, completion, `--version`, and the continuous logs of `serve` are not data responses. `prx schema-version` is a metadata command that emits only `{"schema_version":"2"}` and does not open SQLite, YAML configuration, or GitHub credentials, so a caller can discover the envelope version before decoding other responses.
