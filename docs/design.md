...
Every response-producing CLI command uses schema version `1`. With `--json`, a successful command returns an envelope such as `{"schema_version":"1","ok":true,"data":{...}}`; `data` is always a JSON object. Existing object payloads keep their fields, while collection responses use named fields: `features`, `tasks`, `dependencies`, `pull_requests`, `documents`, `ready_tasks`, `review_waiting_tasks`, `conflict_tasks`, `stale_tasks`, `hosts`, or `auth_methods` as appropriate. A failed command returns `{"schema_version":"1","ok":false,"error":"..."}` with a non-empty top-level error string and no `data`; a successful response has no `error`. The process exits `0` on success and `1` on command failure. Domain error codes remain available inside the application and ConnectRPC contracts, but are not part of the CLI envelope.

Successful normal output returns the `data` object directly as one-line compact JSON, while `--json` wraps the same object in a compact schema envelope; both keep the response on stdout, while warnings and server logs go to stderr. Error responses retain their schema envelope and existing stdout/stderr behavior. Help, completion, `--version`, and the continuous logs of `serve` are not data responses. `prx schema-version` is a metadata command that emits only `{"schema_version":"1"}` in compact JSON and does not open SQLite, YAML configuration, or GitHub credentials, so a caller can discover the envelope version before decoding other responses.
