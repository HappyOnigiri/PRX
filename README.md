...
```sh
make generate          # sqlc, Buf lint, Go/TypeScript protobuf, and CLI reference generation
make generated-check  # regeneration must produce no diff
make mod-tidy-check   # go.mod and go.sum must be tidy
make lint              # go vet, golangci-lint, ESLint, strict TypeScript, and function size/complexity limits
