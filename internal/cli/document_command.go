...
func (s *state) documentCommand() *cobra.Command {
	...
	add := &cobra.Command{Use: "add", Short: "Add a URL or local Markdown path", Example: "prx document add --task TASK_ID --kind markdown_path --value docs/checkout.md --json", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
	...
	deleteCmd := &cobra.Command{Use: "delete DOCUMENT_ID", Short: "Delete a document; missing documents return not_found", Example: "prx document delete DOCUMENT_ID --json", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		if err := s.service.DeleteDocument(cmd.Context(), args[0]); err != nil {
			return err
		}
		return s.write(map[string]string{"deleted": args[0]})
	}}
	list := &cobra.Command{Use: "list", Short: "List URL and local Markdown documents", Example: "prx document list --json", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
