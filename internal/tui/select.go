// Package tui は CLI で共有する対話的な選択画面を提供する。
package tui
	...
	if m.finished {
		if m.confirmed {
			return fmt.Sprintf(
				...
				singleLine(m.selection.Options[m.cursor].Label),
			)
		}
...
// green は選択中の行を緑にする。bubbletea が端末の色プロファイルに合わせて落とすため、
// 色を扱えない端末に制御列は残らない。
func green(value string) string {
	return "\x1b[32m" + value + "\x1b[39m"
}

// singleLine は選択肢に含まれる制御文字を除き、画面の制御シーケンスとして扱わせない。
