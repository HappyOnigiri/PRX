...
func TestRPCReadsOnlyRegisteredMarkdownDocuments(t *testing.T) {
	...
	task, err := client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "Documented task", Kind: prxv1.TaskKind_TASK_KIND_MANUAL}))
	if err != nil {
		t.Fatal(err)
	}
	...
	document, err := client.AddDocument(ctx, connect.NewRequest(&prxv1.AddDocumentRequest{TaskId: task.Msg.GetTask().GetId(), Kind: prxv1.DocumentKind_DOCUMENT_KIND_MARKDOWN_PATH, Title: "Plan", Value: path}))
	if err != nil {
		t.Fatal(err)
	}
	preview, err := client.ReadMarkdownDocument(ctx, connect.NewRequest(&prxv1.ReadMarkdownDocumentRequest{Id: document.Msg.GetDocument().GetId()}))
	if err != nil {
		t.Fatal(err)
	}
	...
	urlDocument, err := client.AddDocument(ctx, connect.NewRequest(&prxv1.AddDocumentRequest{TaskId: task.Msg.GetTask().GetId(), Kind: prxv1.DocumentKind_DOCUMENT_KIND_URL, Title: "Runbook", Value: "https://example.com/runbook"}))
	if err != nil {
		t.Fatal(err)
	}
	_, err = client.ReadMarkdownDocument(ctx, connect.NewRequest(&prxv1.ReadMarkdownDocumentRequest{Id: urlDocument.Msg.GetDocument().GetId()}))
	...
	missingDocument, err := client.AddDocument(ctx, connect.NewRequest(&prxv1.AddDocumentRequest{TaskId: task.Msg.GetTask().GetId(), Kind: prxv1.DocumentKind_DOCUMENT_KIND_MARKDOWN_PATH, Title: "Missing", Value: filepath.Join(t.TempDir(), "missing.md")}))
	if err != nil {
		t.Fatal(err)
	}
	_, err = client.ReadMarkdownDocument(ctx, connect.NewRequest(&prxv1.ReadMarkdownDocumentRequest{Id: missingDocument.Msg.GetDocument().GetId()}))
	...
	largeDocument, err := client.AddDocument(ctx, connect.NewRequest(&prxv1.AddDocumentRequest{TaskId: task.Msg.GetTask().GetId(), Kind: prxv1.DocumentKind_DOCUMENT_KIND_MARKDOWN_PATH, Title: "Large", Value: largePath}))
	if err != nil {
		t.Fatal(err)
	}
	_, err = client.ReadMarkdownDocument(ctx, connect.NewRequest(&prxv1.ReadMarkdownDocumentRequest{Id: largeDocument.Msg.GetDocument().GetId()}))
...
func TestRPCSharesDomainValidation(t *testing.T) {
	...
	a, err := client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "A", Kind: prxv1.TaskKind_TASK_KIND_PULL_REQUEST}))
	if err != nil {
		t.Fatal(err)
	}
	b, err := client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "B", Kind: prxv1.TaskKind_TASK_KIND_PULL_REQUEST}))
	if err != nil {
		t.Fatal(err)
	}
	if _, err = client.AddDependency(ctx, connect.NewRequest(&prxv1.AddDependencyRequest{BlockerTaskId: a.Msg.GetTask().GetId(), BlockedTaskId: b.Msg.GetTask().GetId()})); err != nil {
		t.Fatal(err)
	}
	_, err = client.AddDependency(ctx, connect.NewRequest(&prxv1.AddDependencyRequest{BlockerTaskId: b.Msg.GetTask().GetId(), BlockedTaskId: a.Msg.GetTask().GetId()}))
	...
	for _, detail := range connectErr.Details() {
		...
		foundCycleDetail = errorDetail.GetCode() == prxv1.DomainErrorCode_DOMAIN_ERROR_CODE_CYCLE && len(errorDetail.GetPath()) >= 3
	}
	if !foundCycleDetail {
	...
	if snapshot.Msg.GetSnapshot().GetTasks()[1].GetKind() != prxv1.TaskKind_TASK_KIND_PULL_REQUEST || snapshot.Msg.GetSnapshot().GetTasks()[1].GetBlockedReason() == nil {
		t.Fatalf("unexpected structured task state: %+v", snapshot.Msg.GetSnapshot().GetTasks()[1])
	}
	if snapshot.Msg.GetSnapshot().GetTasks()[1].GetBlockedReason().GetCode() != prxv1.BlockedReasonCode_BLOCKED_REASON_CODE_WAITING_FOR_BLOCKER {
...
func TestRPCRejectsUnknownEnumValues(t *testing.T) {
	...
	task, err := client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "A", Kind: prxv1.TaskKind_TASK_KIND_MANUAL}))
	if err != nil {
		t.Fatal(err)
	}

	unknownKind := prxv1.TaskKind(999)
	if _, err = client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "B", Kind: unknownKind})); errorDetailCode(t, err) != prxv1.DomainErrorCode_DOMAIN_ERROR_CODE_INVALID_KIND {
		t.Fatalf("unknown task kind err=%v", err)
	}

	unknownTaskStatus := prxv1.TaskStatus(999)
	if _, err = client.UpdateTask(ctx, connect.NewRequest(&prxv1.UpdateTaskRequest{Id: task.Msg.GetTask().GetId(), Status: &unknownTaskStatus})); errorDetailCode(t, err) != prxv1.DomainErrorCode_DOMAIN_ERROR_CODE_INVALID_STATUS {
		t.Fatalf("unknown task status err=%v", err)
	}

	unknownFeatureStatus := prxv1.FeatureStatus(999)
	if _, err = client.UpdateFeature(ctx, connect.NewRequest(&prxv1.UpdateFeatureRequest{Id: feature.Msg.GetFeature().GetId(), Status: &unknownFeatureStatus})); errorDetailCode(t, err) != prxv1.DomainErrorCode_DOMAIN_ERROR_CODE_INVALID_STATUS {
...
func TestRPCReportsDistinctErrorCodesPerCause(t *testing.T) {
	...
	manual, err := client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "Sign off", Kind: prxv1.TaskKind_TASK_KIND_MANUAL}))
	if err != nil {
		t.Fatal(err)
	}
	prTask, err := client.CreateTask(ctx, connect.NewRequest(&prxv1.CreateTaskRequest{FeatureId: feature.Msg.GetFeature().GetId(), Title: "Ship API", Kind: prxv1.TaskKind_TASK_KIND_PULL_REQUEST}))
	if err != nil {
		t.Fatal(err)
	}

	_, err = client.AttachPullRequest(ctx, connect.NewRequest(&prxv1.AttachPullRequestRequest{TaskId: manual.Msg.GetTask().GetId(), Url: "https://github.com/org/repo/pull/42"}))
	...
	_, err = client.UpdateTask(ctx, connect.NewRequest(&prxv1.UpdateTaskRequest{Id: prTask.Msg.GetTask().GetId(), Status: &completed}))
	...
	_, err = client.AddDocument(ctx, connect.NewRequest(&prxv1.AddDocumentRequest{TaskId: manual.Msg.GetTask().GetId(), Kind: prxv1.DocumentKind_DOCUMENT_KIND_URL, Title: "Spec", Value: "ftp://example.com/spec"}))
	...
	_, err = client.AddDocument(ctx, connect.NewRequest(&prxv1.AddDocumentRequest{TaskId: manual.Msg.GetTask().GetId(), Kind: prxv1.DocumentKind_DOCUMENT_KIND_URL, Title: "Spec", Value: "  "}))
