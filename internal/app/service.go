...
func (s *Service) SeedDemo(ctx context.Context, slug string, count int) error {
	...
	if err != nil {
		...
		feature, err = s.ResolveFeature(ctx, slug)
		if err != nil {
			return err
		}
	}
	...
	for i := 0; i < count; i++ {
		tasks[i], err = s.CreateTask(ctx, feature.ID, fmt.Sprintf("Delivery slice %02d", i+1), "Implement and verify repository boundary", domain.TaskKindPR, []string{"Ari", "Mika", "Ren"}[i%3])
		if err != nil {
			return err
		}
		_, err = s.AttachPullRequest(ctx, tasks[i].ID, fmt.Sprintf("https://github.com/HappyOnigiri/%s/pull/%d", slug, i+1))
		if err != nil {
			return err
		}
	}
